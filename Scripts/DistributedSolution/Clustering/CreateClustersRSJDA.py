# -*- coding: utf-8 -*-
"""
Created on Fri Sep 29 13:57:19 2023
...
clust_dict = clusteringRSJDA(deltas,number_of_clusters,neutral,m,factor=0.8)
...
for i in range(num_clusters):
    if len(ctr_list[i])>0:
        np.save('RSJDA_'+str(num_clusters_nonempty)+'_vtx_cluster_'+str(counter)+'.npy',vrtcs_list[i])
        np.save('RSJDA_'+str(num_clusters_nonempty)+'_ctr_cluster_'+str(counter)+'.npy',ctr_list[i])
        counter += 1